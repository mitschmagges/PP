...
void savebmp(char *name, uchar *buffer, int x, int y) {
	...
	unsigned int size=x*y*3+54; //PixelNumber * 3Colors + (bmpHeader)
...
void fancycolour(uchar *p,int iter) {
	if(iter==MAXITER);
		...
		p[0]=128+iter*16; p[1]=p[2]=0;
	} else if(iter<24) {
		p[0]=255; p[1]=p[2]=(iter-8)*16;
	} else if(iter<160) {
		p[0]=p[1]=255-(iter-24)*2; p[2]=255;
	} else {
		p[0]=p[1]=(iter-160)*2; p[2]=255-(iter-160)*2;
	}
}